...
#define SERVO_MOTOR_L_MAX (113)  // 定义主板上舵机活动范围(左打方向的极限值) 角度 自行标定，以98为中心，这里就是直接设置的舵机旋转的角度
#define SERVO_MOTOR_R_MAX (83) // 定义主板上舵机活动范围(右打方向的极限值) 角度 自行标定
#define SERVO_DIR (SERVO_MOTOR_L_MAX>SERVO_MOTOR_R_MAX?-1.f:1.f) // 就是根据L_MAX与R_MAX设置舵机旋转的方向
